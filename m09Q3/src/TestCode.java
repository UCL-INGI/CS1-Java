...
public class TestCode{
	...
	private class t2 implements Callable<Void> {
		...
		public Void call() throws Exception, IOException{
			...
			try {
				...
				String feed = MessageFormat.format(Translator.translate("{0} : vous ne faîtes pas appel à close lorsque la méthode se termine sans erreur !"),test_name());
				fail(feed);
				return null;
			}
		}
	...
	private class t3 implements Callable<Void> {
		...
		public Void call() throws ConstructException,IOException {
			...
			String feed = MessageFormat.format(Translator.translate("{0} : votre code ne compte pas le bon nombre de lignes !"),test_name());
			if(count1 != 79)
				fail(feed);
			if(count2 != 5)
				fail(feed);
	...
	private class t4 implements Callable<Void> {
		...
		public Void call() throws Exception,IOException{
			...
			try {
				...
				String feed = MessageFormat.format(Translator.translate("{0} : vous ne renvoyez pas -1 lorsqu''une erreur se produit !"),test_name());
