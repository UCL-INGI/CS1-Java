...
public class Tests {
    public void testd2i(String s) throws IllegalAccessException, IllegalArgumentException, InvocationTargetException {
        String feedback  = Translator.translate("Avec l''argument s=\"{0}\", la réponse attendue est\n{1}\nMalheureusement, votre code renvoie \n{2}\n");
        int result = Correction.d2i(s);
        int resultStudent = (int) FunctionHelper.run_student_function("d2i", s);
    ...
    public void testLauncher(){
        Random r = new Random();
        try{
            FunctionHelper.check_etudiant_function("StudentCode.Etudiant", "d2i", int.class, new Class[]{String.class});
            for(int i = 10; i > 0; i--){
