...
public class StringHelper {
	/**
	 * 	@pre 
	 * 	@post	Renvoie un string contenant des caractères qui se répètent au maximum répétitions fois sur base de base. exemple: aabbbcccc
			max(length(valeurRetour)) = repetition * taille  
	 */
	public static String getCharRep(int repetition, int taille, String base){
		String toReturn = "";
		Random r = new Random();
	...
	public static String getRandomSubstring(String s){
		Random r = new Random();
		return s.substring(r.nextInt(s.length()/2), r.nextInt(s.length()/2) + ((s.length()/2) - 1));
	}
	/**
	 * 	@pre 
	 * 	@post	Génère un string contenant des caractères qui se répètent au maximum répétitions fois. exemple: aabbbcccc
	...
	/**
	 * 	@pre	-
	 * 	@post	Génère un String aléatoirement, avec des caractères différents, de taille length, soit composé de lettres alphabétique soit de chiffres.
	 */
