...
public class Tests {
	...
	public void testRemoveOther(){
		...
		try{
			...
			student.Process p = new student.Process("firefox", 0);
			student.Process pOther = new student.Process("init", 0);
			bc.addProcess(p);
			boolean b;
			...
			for(int i = 0 ; i < 20 ; i++){
				p = new student.Process(generateString(r.nextInt(7) + 3), 0);
				bc.addProcess(p);
				pOther = new student.Process(generateString(r.nextInt(7) + 3), 0);
				b = bc.removeProcess(pOther);
	...
	public void testRemoveOtherSameAttributes(){
		...
		try{
			BasicComputer bc = new BasicComputer("ordi1");
			student.Process p = new student.Process("firefox", 0);
			student.Process pOther = new student.Process("firefox", 0);
			bc.addProcess(p);
			boolean b;
			b = bc.removeProcess(pOther);
			...
			for(int i = 0 ; i < 20 ; i++){
				...
				p = new student.Process(name, 0);
				bc.addProcess(p);
				pOther = new student.Process(name, 0);
				b = bc.removeProcess(pOther);
				assertFalse(str + "removeProcess a renvoyé true lorsqu'on lui demande de retirer le processus dont le pid est "
